# 自定义域名配置指南 - cloudnorth.cloud

本指南专门针对 `cloudnorth.cloud` 域名的 Vercel 配置。
...
4. 在 "Add Domain" 输入框中输入：`cloudnorth.cloud`
5. 点击 **Add**
...
```
类型: A
主机记录: @ (或留空，表示根域名 cloudnorth.cloud)
...
1. 登录 Namecheap 账户
2. 进入 **Domain List** → 选择 `cloudnorth.cloud`
3. 点击 **Advanced DNS**
...
1. **在 Vercel 中查看状态**
   ...
   - 查看 `cloudnorth.cloud` 的状态
   - 当显示 "Valid Configuration" 时，配置成功

2. **使用命令行检查**
   ```bash
   # 检查 A 记录
   dig cloudnorth.cloud A
   
   # 或使用 nslookup
   nslookup cloudnorth.cloud
   ```
...
- ✅ `https://cloudnorth.cloud` - 应该正常访问
- ✅ `http://cloudnorth.cloud` - 会自动重定向到 HTTPS
...
- [ ] 在 Vercel 中添加了 `cloudnorth.cloud` 域名
- [ ] 在域名注册商配置了正确的 DNS 记录
- [ ] 等待 DNS 记录生效（5-60 分钟）
- [ ] Vercel 显示 "Valid Configuration"
- [ ] SSL 证书已配置完成
- [ ] 可以通过 `https://cloudnorth.cloud` 访问网站
- [ ] HTTP 自动重定向到 HTTPS
